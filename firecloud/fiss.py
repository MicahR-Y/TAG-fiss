...
@fiss_cmd
def bucket_use(args):
    ''' Retrieve bucket usage for a workspace'''
    r = fapi.get_bucket_usage(args.project, args.workspace)
    fapi._check_response_code(r, 200)
    #result = dict()
    #result = r.json()['usageInBytes']
    return r.text

@fiss_cmd
def storage_cost(args):
    ''' Retrieve monthly storage cost for a workspace'''
...
def main(argv=None):
    ...
    subp = subparsers.add_parser(
        'bucket_use', description='Show bucket usage of workspace',
        parents=[workspace_parent])
    subp.set_defaults(func=bucket_use)

    subp = subparsers.add_parser(
        'storage_cost', description='Show bucket usage of workspace',
        parents=[workspace_parent])
    subp.set_defaults(func=storage_cost)
